...
@if (session('success'))
    <div class="mb-4 bg-emerald-50 border border-emerald-200 rounded-xl px-5 py-3 text-sm text-emerald-800">
        {{ session('success') }}
    </div>
@endif

<p class="mb-4 text-xs text-gray-500">
    Isikan bobot (%) untuk setiap teknik penilaian per CPMK. Total bobot per CPMK sebaiknya berjumlah <strong>100%</strong>.
</p>
        ...
        <table class="border-collapse text-xs">
            <thead>
                <tr style="background:#059669">
                    ...
                    <th class="px-4 py-3 text-left font-bold text-white border border-emerald-400" style="min-width:180px">Nama MK</th>
                    <th class="px-3 py-3 text-center font-bold text-white border border-emerald-400 w-28">CPMK</th>
                    <th class="px-3 py-3 text-center font-bold text-white border border-emerald-400 w-20">Partisipasi<br><span class="font-normal text-emerald-100">(Quiz)</span></th>
                    <th class="px-3 py-3 text-center font-bold text-white border border-emerald-400 w-20">Observasi<br><span class="font-normal text-emerald-100">(Praktik)</span></th>
                    <th class="px-3 py-3 text-center font-bold text-white border border-emerald-400 w-20">Unjuk Kerja<br><span class="font-normal text-emerald-100">(Presentasi)</span></th>
                    <th class="px-3 py-3 text-center font-bold text-white border border-emerald-400 w-20">Tes Tulis<br><span class="font-normal text-emerald-100">(UTS)</span></th>
                    <th class="px-3 py-3 text-center font-bold text-white border border-emerald-400 w-20">Tes Tulis<br><span class="font-normal text-emerald-100">(UAS)</span></th>
                    <th class="px-3 py-3 text-center font-bold text-white border border-emerald-400 w-20">Tes Lisan<br><span class="font-normal text-emerald-100">(Kelompok)</span></th>
                    ...
                    @foreach ($cpmks as $ci => $cpmk)
                        ...
                        <tr class="bobot-row" style="background:{{ $rowBg }}" data-cpmk="{{ $cpmk->id }}">
                                ...
                                <td class="border border-emerald-100 text-center align-middle px-1 py-1">
                                    @if (!$kurikulum->isArsip())
                                        <input type="number"
                                               ...
                                               class="bobot-input w-16 text-center border border-gray-300 rounded px-1 py-0.5 text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500"
                                               placeholder="0">
                                    @else
                                        <span class="text-gray-700">{{ $val !== '' ? $val : '—' }}</span>
                                    @endif
                                </td>
                            @endforeach
                            <td class="border border-emerald-100 text-center align-middle px-2">
                                ...
                                @else
                                    @php
                                        $tot = (float)($p->bobot_quiz ?? 0) + (float)($p->bobot_observasi ?? 0)
                                             + (float)($p->bobot_unjuk_kerja ?? 0) + (float)($p->bobot_uts ?? 0)
                                             + (float)($p->bobot_uas ?? 0) + (float)($p->bobot_tes_lisan ?? 0);
...
<div class="mt-4 flex items-center gap-3">
    <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium px-6 py-2 rounded-lg transition-colors">

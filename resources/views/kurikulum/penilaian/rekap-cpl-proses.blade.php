...
@section('title', 'Proses Penilaian dan Evaluasi CPL')
@section('header', 'Proses Penilaian dan Evaluasi CPL')
...
@section('breadcrumb')
    ...
    <span class="mx-1 text-gray-300">/</span>
    <span class="text-gray-700 font-medium">Proses Penilaian dan Evaluasi CPL</span>
@endsection

@section('header-actions')
    @if ($semester && $selectedCpl)
        @include('layouts._export-button', ['route' => route('kurikulum.penilaian.evaluasi-cpl.export', array_merge(['kurikulum' => $kurikulum, 'semester' => $semester->id, 'cpl' => $selectedCpl->id], array_filter($filters)))])
    @endif
@endsection
...
<form method="GET" class="flex flex-wrap items-end gap-3 mb-4">
    ...
    <div>
        <label for="cpl" class="block text-xs font-medium text-gray-600 mb-1">CPL Prodi:</label>
        <select name="cpl" id="cpl" onchange="this.form.submit()"
            ...
            @forelse ($cplOptions as $cplOpt)
                <option value="{{ $cplOpt->id }}" {{ $selectedCpl && $selectedCpl->id === $cplOpt->id ? 'selected' : '' }}>
                    {{ $cplOpt->kode_cpl }} — {{ Str::limit($cplOpt->deskripsi, 60) }}
                </option>
            @empty
                <option value="">— Belum ada CPL —</option>
            @endforelse
        </select>
    </div>
    <div>
...
<div class="flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 mb-3 text-xs text-blue-800">
    ...
    <p class="flex-1">
        Tabel ini merekap proses penilaian dan evaluasi untuk <strong>satu CPL Prodi</strong> secara lintas Mata Kuliah
        dalam satu semester. Pilih CPL pada dropdown di atas untuk melihat bagaimana nilai CPMK dari
        tiap Mata Kuliah dikombinasikan menjadi Nilai Mata Kuliah, lalu diagregasi menjadi Capaian CPL.
        Gunakan <strong>Export Excel</strong> untuk mengunduh tabel ini sebagai template, lalu unggah kembali
        file yang sudah diisi melalui <strong>Import Excel</strong> di bawah untuk memperbarui nilai setiap kolom
        CPMK. Kolom "Nilai Mata Kuliah", "Nilai CPL", dan "Capaian CPL" tidak perlu diisi karena dihitung ulang
        secara otomatis setelah import.
    </p>
</div>
...
@elseif ($cplOptions->isEmpty())
    <div class="bg-amber-50 border border-amber-200 rounded-xl px-5 py-5 text-sm text-amber-800 flex items-start gap-3">
        ...
        <div>
            ...
            <p class="text-xs mt-0.5">Belum ada CPL Prodi dengan CPMK + Sub-CPMK + Komponen Asesmen pada semester {{ $semester->nama }} {{ $semester->tahun_akademik }}.</p>
        </div>
    </div>
@elseif (empty($mkGroups))
    <div class="bg-amber-50 border border-amber-200 rounded-xl px-5 py-5 text-sm text-amber-800">
        Belum ada data Mata Kuliah/CPMK untuk CPL {{ $selectedCpl->kode_cpl }} pada semester ini.
    </div>
@elseif ($mahasiswaList->isEmpty())
    <div class="bg-amber-50 border border-amber-200 rounded-xl px-5 py-5 text-sm text-amber-800">
        Belum ada mahasiswa yang terdaftar (enrollment aktif) pada mata kuliah-mata kuliah terkait CPL ini.
    </div>
@else

{{-- Import form --}}
<div class="mb-4 bg-white border border-gray-100 rounded-xl shadow-sm px-4 py-3">
    <form method="POST" action="{{ route('kurikulum.penilaian.evaluasi-cpl.import', $kurikulum) }}" enctype="multipart/form-data" class="flex flex-wrap items-center gap-3">
        @csrf
        <input type="hidden" name="semester" value="{{ $semester->id }}">
        <input type="hidden" name="cpl" value="{{ $selectedCpl->id }}">

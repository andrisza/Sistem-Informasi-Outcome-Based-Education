        ...
        <table class="w-full text-sm border-collapse obe-list-table">
            <thead>
                <tr style="background:#F59E0B">
                    ...
                    <th class="px-4 py-3 text-left text-xs font-bold text-white border border-amber-300 w-40">Kategori</th>
                    ...
                    <tr class="border-b border-amber-100 hover:bg-amber-50 transition-colors" style="{{ $loop->even ? 'background:#fffbeb' : 'background:#fff' }}">
                        ...
                        <td class="px-4 py-3 border border-amber-100">
                            @if ($pl->kategori)
                                @php
                                    $kCls = match(true) {
                                        str_contains($pl->kategori, 'Penciri')       => 'bg-blue-100 text-blue-800',
                                        ...
                                        str_contains($pl->kategori, 'tambahan')      => 'bg-orange-100 text-orange-800',
                        ...
                        <td class="px-4 py-3 text-center border border-amber-100">
                            @if ($pl->cpl_prodi_count > 0)
                                <span class="inline-flex items-center justify-center w-7 h-7 rounded-full bg-amber-100 text-amber-800 text-xs font-bold">{{ $pl->cpl_prodi_count }}</span>
                            @else

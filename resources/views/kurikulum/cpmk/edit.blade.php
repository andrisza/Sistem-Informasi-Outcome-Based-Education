...
@section('title', 'Edit CPMK – ' . $cpmk->kode_cpmk)
@section('header', 'Edit CPMK')
...
@section('breadcrumb')
    ...
    <span class="mx-1 text-gray-300">/</span>
    <span class="text-gray-700 font-medium">{{ $cpmk->kode_cpmk }}</span>
@endsection

@section('content')

<form method="POST" action="{{ route('kurikulum.mata-kuliah.cpmk.update', [$kurikulum, $mataKuliah, $cpmk]) }}" class="max-w-2xl">
    @csrf @method('PUT')
    @if (request('_redirect_to'))
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm divide-y divide-gray-50">
        <div class="px-6 py-5 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="kode_cpmk" value="{{ old('kode_cpmk', $cpmk->kode_cpmk) }}"
                       class="w-full border border-gray-300 rounded-lg px-3.5 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500 @error('kode_cpmk') border-red-400 @enderror"
                       placeholder="cth: CPMK011">
                <p class="text-[11px] text-gray-400 mt-1">Format: CPMK + kode CPL (2 digit) + urutan (1 digit)</p>
            ...
            <div>
                ...
                <select name="id_cpl"
                    ...
                    @foreach ($cplList as $cpl)
                        <option value="{{ $cpl->id }}" {{ old('id_cpl', $cpmk->id_cpl) == $cpl->id ? 'selected' : '' }}>
                ...
                <select name="level_bloom"
                    ...
                    <optgroup label="Kognitif (C)">
                        @foreach (['C1','C2','C3','C4','C5','C6'] as $lvl)
                            <option value="{{ $lvl }}" {{ old('level_bloom', $cpmk->level_bloom) === $lvl ? 'selected' : '' }}>{{ $lvl }}</option>
                        @endforeach
                    </optgroup>
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4"
                          ...
                          placeholder="Deskripsi CPMK...">{{ old('deskripsi', $cpmk->deskripsi) }}</textarea>
        ...
        <div class="px-6 py-4 flex items-center gap-3">
            ...
            <a href="{{ request('_redirect_to') ?: route('kurikulum.mata-kuliah.cpmk.index', [$kurikulum, $mataKuliah]) }}"

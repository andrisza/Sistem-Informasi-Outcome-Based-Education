...
@section('header', 'Tambah Periode Kurikulum')
...
<form method="POST" action="{{ route('kurikulum.periode.store', $kurikulum) }}" class="max-w-2xl">
    @csrf
        ...
        <div class="px-6 py-5 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nama_periode" value="{{ old('nama_periode') }}"
                       ...
                       placeholder="cth: Penyusunan Kurikulum 2021">
                ...
                <div>
                    ...
                    <select name="ketua_tim"
                        ...
                        @foreach ($dosenList as $dosen)
                            <option value="{{ $dosen->id }}" {{ old('ketua_tim') == $dosen->id ? 'selected' : '' }}>{{ $dosen->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
            ...
            <div>
                ...
                <input type="text" name="dokumen_sk" value="{{ old('dokumen_sk') }}"
                       class="w-full border border-gray-300 rounded-lg px-3.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"

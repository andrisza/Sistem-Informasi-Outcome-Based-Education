...
<form method="POST" action="{{ route('kurikulum.arsip-rapat.store', $kurikulum) }}" class="max-w-2xl">
    @csrf
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm divide-y divide-gray-50">
        <div class="px-6 py-5 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="judul_rapat" value="{{ old('judul_rapat') }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="jenis_rapat"
                        ...
                        @foreach ($jenisOptions as $val => $label)
                            <option value="{{ $val }}" {{ old('jenis_rapat') == $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_rapat" value="{{ old('tanggal_rapat') }}"
            ...
            <div>
                ...
                <textarea name="agenda" rows="3"
                          class="w-full border border-gray-300 rounded-lg px-3.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                          placeholder="Agenda rapat...">{{ old('agenda') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="notulen" rows="4"
                          class="w-full border border-gray-300 rounded-lg px-3.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                          placeholder="Notulen rapat...">{{ old('notulen') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="tindak_lanjut" rows="3"
                          class="w-full border border-gray-300 rounded-lg px-3.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                          placeholder="Tindak lanjut...">{{ old('tindak_lanjut') }}</textarea>
            </div>

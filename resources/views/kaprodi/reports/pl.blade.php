...
@section('title', 'Laporan PL – ' . $kurikulum->nama_kurikulum)
@section('header', 'Ketercapaian PL')

@section('breadcrumb')
    <a href="{{ route('kaprodi.reports.index') }}" class="hover:text-blue-600">Laporan</a>
    <span class="mx-1 text-gray-300">/</span>
    <span class="text-gray-700 font-medium">PL · {{ $kurikulum->kode }}</span>
@endsection
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-5">
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
        ...
        <div>
            ...
            <p class="font-medium text-gray-800 mt-0.5">{{ $hasilPl->count() }} baris</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
    <div class="px-5 py-4 border-b border-gray-100">
        <h4 class="font-semibold text-gray-800 text-sm">Ketercapaian Profil Lulusan per Semester</h4>
        <p class="text-xs text-gray-400 mt-0.5">Rata-rata nilai PL seluruh mahasiswa, dikelompokkan per PL dan semester.</p>
    </div>
    @if ($hasilPl->count())
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-100">
                    <th class="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Profil Lulusan</th>
            ...
            <tbody class="divide-y divide-gray-50">
                @foreach ($hasilPl as $hasil)
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-5 py-3">
                            <span class="font-mono font-semibold text-blue-700 text-xs">{{ $hasil->kode_pl }}</span>
                            @if ($hasil->pl_deskripsi)
                                <p class="text-xs text-gray-400 mt-0.5 max-w-xs">{{ Str::limit($hasil->pl_deskripsi, 60) }}</p>
                            @endif
                        </td>
    ...
    @else
        <div class="px-5 py-12 text-center text-gray-400 text-sm">
            Belum ada data hasil PL untuk kurikulum ini.
        </div>
    @endif
</div>

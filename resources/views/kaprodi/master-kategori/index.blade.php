...
@if (session('success'))
    <div class="mb-4 flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl px-4 py-3 text-sm text-green-800">
    ...
    @foreach ([
        ...
        <button type="button"
                ...
                class="px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors duration-150
                       {{ $tabKey === 'pl' ? 'border-amber-500 text-amber-700' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
            {{ $tabLabel }}
            <span class="ml-1.5 inline-flex items-center justify-center w-5 h-5 rounded-full text-xs font-bold
                         {{ $tabKey === 'pl' ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-600' }}"
                  id="tab-count-{{ $tabKey }}">
...
<div id="tab-panel-{{ $tabKey }}" class="{{ $tabKey === 'pl' ? '' : 'hidden' }}">

    <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-6">

        {{-- Table header --}}
        <div style="background:#F59E0B" class="px-5 py-3 flex items-center justify-between">
            <span class="text-white font-bold text-sm">Kategori — {{ $tabLabel }}</span>
                    ...
                    @forelse ($grouped->get($tabKey, collect()) as $kat)
                        ...
                        <tr id="row-{{ $kat->id }}" class="border-b border-amber-100 hover:bg-amber-50 transition-colors"
                            ...
                            <td class="px-3 py-3 border border-amber-100">
                                <div class="flex items-center justify-center gap-1.5">
                                    ...
                                    <form method="POST" action="{{ route('kaprodi.master-kategori.toggle', $kat) }}"
                        ...
                        <tr id="edit-row-{{ $kat->id }}" class="hidden bg-blue-50 border-b border-blue-200">
                            <td colspan="5" class="px-4 py-3 border border-blue-200">
                                <form method="POST" action="{{ route('kaprodi.master-kategori.update', $kat) }}" class="flex items-end gap-3 flex-wrap">
                                    @csrf
                                    @method('PUT')
        ...
        <div class="px-5 py-4 border-t border-gray-100 bg-gray-50">
            ...
            <form method="POST" action="{{ route('kaprodi.master-kategori.store') }}" class="flex items-end gap-3 flex-wrap">
                @csrf

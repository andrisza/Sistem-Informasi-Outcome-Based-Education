...
@section('header', 'Tambah Jurnal Mengajar')
...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        <form method="POST" action="{{ route('dosen.jurnal.store') }}" class="space-y-5">
            @csrf
            ...
            <div>
                    ...
                    @foreach ($rpsList as $rps)
                        ...
                        <optgroup label="{{ $rps->mataKuliah->nama_mk ?? 'MK' }} — {{ $rps->semester->nama ?? '' }}">
                            @foreach ($rps->pertemuan->sortBy('minggu_ke') as $pt)
                                <option value="{{ $pt->id }}"
                                        {{ old('id_rps_pertemuan') == $pt->id ? 'selected' : '' }}>
                ...
                @if($rpsList->isEmpty())
                    <p class="mt-1 text-xs text-amber-600">
                        Belum ada RPS dengan status review/disahkan. Harap submit RPS terlebih dahulu.
                    </p>
                @endif
            ...
            <div>
                ...
                <input type="date" name="tanggal_pelaksanaan"
                       value="{{ old('tanggal_pelaksanaan', now()->format('Y-m-d')) }}" required
                       class="w-full border rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500
            ...
            <div>
                ...
                <input type="number" name="jumlah_hadir" min="0"
                       value="{{ old('jumlah_hadir', 0) }}" required
                       class="w-full border rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500
            ...
            <div>
                ...
                <textarea name="realisasi_materi" rows="4" required
                          placeholder="Deskripsikan materi yang sebenarnya disampaikan pada pertemuan ini..."
                          class="w-full border rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500
                                 {{ $errors->has('realisasi_materi') ? 'border-red-400' : 'border-gray-300' }}">{{ old('realisasi_materi') }}</textarea>
            ...
            <div>
                ...
                <textarea name="catatan" rows="3"
                          placeholder="Catatan tambahan, kendala, atau hal lain yang perlu dicatat..."
                          class="w-full border border-gray-300 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">{{ old('catatan') }}</textarea>
            </div>
